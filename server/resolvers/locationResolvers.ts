...
const locationResolver = {
  Query: {
    ...
    userLocations: async (_root: User, _args: User, context: MyContext) => {
      ...
      return LocationtModel.find({ _id: { $in: userLocations } }).populate('admin')
    },
  ...
  Mutation: {
    ...
    editLocation: async (
      ...
      try {
        const newLocation = await LocationtModel.findByIdAndUpdate(args.id, args, { new: true })
        if (args.admin) {
    ...
    deleteLocation: async (_root: Location, _args: Location, context: MyContext) => {
      if (!context.isAuthenticated()) {
        throw new GraphQLError('wrong credentials', {
          extensions: { code: 'UNAUTHORIZED' },
        })
      ...
      const location: Location | null = await LocationtModel.findById(currentLocation).populate(
        'admin'
      )
      ...
      try {
        ...
        context.req.session.currentLocation = currentUser?.permissions[0].location || undefined
