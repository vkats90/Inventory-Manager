...
const TopBar = () => {
  ...
  return (
    <div className={`relative w-full flex flex-row m-0  px-5  h-10 z-30 bg-slate-300`}>
      <Hamburger />
      <div
        id="logo"
      ...
      >
        ...
        {location.pathname != '/products' && (
          <Link className="self-center cursor-pointer hover:text-primary" to="/products">
            Products
          </Link>
        )}
        {location.pathname != '/parts' && (
          <Link className="self-center cursor-pointer hover:text-primary" to="/parts">
            Parts
          </Link>
        )}
        {location.pathname != '/orders' && (
          <Link className="self-center cursor-pointer hover:text-primary" to="/orders">
            Orders
          </Link>
        )}
      </div>
      <div id="social" className="flex-none flex flex-row justify-end items-stretch gap-4"></div>
    </div>
  )
}
